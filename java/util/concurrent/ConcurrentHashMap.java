...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            if (tab == null || (n = tab.length) == 0)
                ...
                // 如果CAS成功，说明Node节点已经插入，随后addCount(1L,binCout)方法会检查当前容量是否需要进行扩容。
    ...
    /**
     * 一个特殊的Node节点，hash值为-1，其中存储nextTable的引用。只有table发生扩容的时候，
     * ForwardingNode才会发挥作用，作为一个占位符放在table中表示当前节点为null或则已经被移动。
